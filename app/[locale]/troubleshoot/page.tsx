import { getTroubleshootEntries } from "@/lib/troubleshoot-data";
...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return {
    title: `${dict.troubleshoot.title} — ${dict.meta.siteName}`,
    description: dict.troubleshoot.subtitle,
  };
}

export default async function TroubleshootPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: rawLocale } = await params;
  if (!isLocale(rawLocale)) notFound();
  const locale: Locale = rawLocale;
  const dict = getDictionary(locale);
  const t = dict.troubleshoot;
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-20">
      ...
      <div className="mt-10">
        <TroubleshootExplorer entries={getTroubleshootEntries(locale)} dict={dict} locale={locale} />
      </div>
    </div>
  );
}
