...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }) {
  ...
  return { title: `${dict.filters.title} — ${dict.meta.siteName}` };
}

export default async function FiltersToolPage({ params }: { params: Promise<{ locale: string }> }) {
  const { locale: rawLocale } = await params;
  ...
  return (
    <div className="mx-auto max-w-4xl px-5 py-20">
      ...
      <div className="mt-10">
        <FilterRangeHelper locale={locale} dict={dict} />
      </div>
    </div>
  );
}
